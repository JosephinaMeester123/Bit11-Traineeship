...
panel.fill<-function(x,y,mychr,CBSsegments){
  ...
  diagnosticBand <- read.csv(diagBandcsv,header=TRUE)
  cytoBand <- read.csv(cytoBandcsv,header=TRUE)
...
ref_data <- read.table(ref_bin_file,as.is=T,header=T,sep=",")
...
achrom = bin_data[which(bin_data$CHR != "X" & bin_data$CHR != "Y"),]	#autosomes
...
if(seqType=="NextSeq"){
	xchrom = bin_data[bin_data$CHR == "X",]
	xchrom_full <- merge(xchrom,gender_ref_data,by=c('CHR','BIN.START','BIN.END'))
	if(!(is.na(genders[tolower(gender)]))){
		if(genders[tolower(gender)]=="M"){
			message("Using male column")
			xchrom_full$ratio = xchrom_full[,sample.gc.auto.X.M.gr10M] / xchrom_full[,"GC.AUTO.X.GR10M.MEAN"]
			xchrom_full$z = (xchrom_full[,sample.gc.auto.X.M.gr10M] - xchrom_full[,"GC.AUTO.X.GR10M.MEAN"]) / xchrom_full[,"GC.AUTO.X.GR10M.SD"]
			xchrom_full$logR = log2(xchrom_full$ratio)
		} else {
			message("Using female column")
			xchrom_full$ratio = xchrom_full[,sample.gc.auto.X.F.gr10M] / xchrom_full[,"GC.AUTO.X.GR10M.MEAN"]
			xchrom_full$z = (xchrom_full[,sample.gc.auto.X.F.gr10M] - xchrom_full[,"GC.AUTO.X.GR10M.MEAN"]) / xchrom_full[,"GC.AUTO.X.GR10M.SD"]
			xchrom_full$logR = log2(xchrom_full$ratio)
		}
	} else {
		#ongekend gender
		message("Unknown, using male column")
		xchrom_full$ratio = xchrom_full[,sample.gc.auto.X.M.gr10M] / xchrom_full[,"GC.AUTO.X.GR10M.MEAN"]
		xchrom_full$z = (xchrom_full[,sample.gc.auto.X.M.gr10M] - xchrom_full[,"GC.AUTO.X.GR10M.MEAN"]) / xchrom_full[,"GC.AUTO.X.GR10M.SD"]
		xchrom_full$logR = log2(xchrom_full$ratio)
	}

	achrom_full$ratio = achrom_full[,sample.gc.auto.gr10M] / achrom_full[,"GC.AUTO.GR10M.MEAN"]
	achrom_full$z = (achrom_full[,sample.gc.auto.gr10M] - achrom_full[,"GC.AUTO.GR10M.MEAN"]) / achrom_full[,"GC.AUTO.GR10M.SD"]
	achrom_full$logR = log2(achrom_full$ratio)

	xCNA <- CNA(as.matrix(xchrom_full$z),xchrom_full$CHR,as.numeric(xchrom_full$BIN.START),data.type="logratio",sampleid=samplename)
	xObj <- segment(xCNA,verbose=0,min.width = min.width.bins,alpha=alpha)
	xSeg <- xObj$output
	xSeg$Sample <- samplename
	...
	CNVtable <- rbind(aSeg,xSeg);

} else if(seqType=="HiSeq"){
	# no gender data
	achrom_full$ratio = achrom_full[,14] / achrom_full[,27]
	achrom_full$z = (achrom_full[,14] - achrom_full[,27]) / achrom_full[,28]
	achrom_full$logR = log2(achrom_full$ratio)
...
listoutput <- paste(output_map,paste(samplename, "CNVs_maternal.txt", sep="."),sep="/")
