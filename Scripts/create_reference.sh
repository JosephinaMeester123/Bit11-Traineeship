...
for filename in /home/jmeester/WGS_Jeannette/Testing/*.count.stdout.*.pbs.biomina.be; do
...
for file in /home/jmeester/WGS_Jeannette/Testing/*.count.stdout.*.pbs.biomina.be.autosomal; do
	totalsample="$(awk '{total += $4} END {print total}' $file)"
	awk -v totalavg="$totalavg" -v totalsample="$totalsample" '{print $1, $2, $3, $4, totalsample, totalavg, $4/totalsample*totalavg}' $file > $file".count.norm"
	awk '{print $7} ' $file".count.norm" > $file".count.norm.countonly"
done

#paste together windows1kb file, which contains locations of bins, with the counts of all samples
paste /home/jmeester/WGS_Jeannette/Testing/locations.txt /home/jmeester/WGS_Jeannette/Testing/*.autosomal.count.norm.countonly > /home/jmeester/Internship/Reference_test/auto_count_allsamples_norm.txt
...
rm /home/jmeester/WGS_Jeannette/Testing/*.autosomal.count.norm.countonly
